...
def get(uri,sessionKey,cert,verify,token=None,headers=None,payload=None,user=None,password=None,timeout=60):
    try:
...
def head(uri,sessionKey,cert,verify,token=None,headers=None,payload=None,user=None,password=None,timeout=60):
    try:
        if sessionKey == None and token == None:
            if user == None and password == None:
...
def patch(uri,sessionKey,cert,verify,token=None,headers=None,payload=None,user=None,password=None,timeout=60):
    try:
        if sessionKey == None and token == None:
            if user == None and password == None:
            ...
            headers = {}
            headers["Authorization"] = "Bearer %s" % token
            r = requests.patch(uri,params=payload,verify=verify,cert=cert,headers=headers,timeout=timeout)                
        else:
            headers = {}
            headers["Authorization"] =  "Splunk %s" % sessionKey
            r = requests.patch(uri,data=payload,verify=verify,cert=cert,headers=headers,timeout=timeout)
...
def post(uri,sessionKey,cert,verify,token=None,headers=None,payload=None,user=None,password=None,timeout=60):
    try:
        if sessionKey == None and token == None:
            if user == None and password == None:
            ...
            headers = {}
            headers["Authorization"] = "Bearer %s" % token
            r = requests.post(uri,params=payload,verify=verify,cert=cert,headers=headers,timeout=timeout)                
        else:
            headers = {}
            headers["Authorization"] =  "Splunk %s" % sessionKey
            r = requests.post(uri,data=payload,verify=verify,cert=cert,headers=headers,timeout=timeout)
...
def put(uri,sessionKey,cert,verify,token=None,headers=None,payload=None,user=None,password=None,timeout=60):
    try:
        if sessionKey == None and token == None:
            if user == None and password == None:
            ...
            headers = {}
            headers["Authorization"] = "Bearer %s" % token
            r = requests.put(uri,params=payload,verify=verify,cert=cert,headers=headers,timeout=timeout)                
        else:
            headers = {}
            headers["Authorization"] =  "Splunk %s" % sessionKey
            r = requests.put(uri,data=payload,verify=verify,cert=cert,headers=headers,timeout=timeout)
...
def delete(uri,sessionKey,cert,verify,token=None,headers=None,payload=None,user=None,password=None,timeout=60):
    try:
        if sessionKey == None and token == None:
            if user == None and password == None:
            ...
            headers = {}
            headers["Authorization"] = "Bearer %s" % token
            r = requests.delete(uri,params=payload,verify=verify,cert=cert,headers=headers,timeout=timeout)                
        else:
            headers = {}
            headers["Authorization"] = 'Splunk %s' % sessionKey
            r = requests.delete(uri,data=payload,verify=verify,cert=cert,headers=headers,timeout=timeout)
        ...
        if 'uri' not in options and 'urifield' not in options:
            ...
            if len(results) > 0:
                ...
                for result in results:
                    ...
                    # Option for TLS-verify 
                    if 'verify' in options:
                        verify = options['verify'].lower()
                        if verify in ("false", "f", "0"):
                            verify = False
                        elif verify.startswith("/") or verify.endswith(".pem"):
                            verify = str(options['verify'])  # if a path is added
                        else:
                            verify = True
                    else:
                    ...
                    if method.lower() in ("get","g"):
                        Result = get(uri,sessionKey,cert,verify,token,headers,data,user,passwd,timeout)
                    if method.lower() in ("head","h"):
                        Result = head(uri,sessionKey,cert,verify,token,headers,data,user,passwd,timeout)
                    if method.lower() in ("patch"):
                        Result = patch(uri,sessionKey,cert,verify,token,headers,data,user,passwd,timeout)
                    if method.lower() in ("post","p"):
                        Result = post(uri,sessionKey,cert,verify,token,headers,data,user,passwd,timeout)
                    if method.lower() in ("put"):
                        Result = put(uri,sessionKey,cert,verify,token,headers,data,user,passwd,timeout)
                    if method.lower() in ("delete","del","d"):
                        Result = delete(uri,sessionKey,cert,verify,token,headers,data,user,passwd,timeout)
            ...
            # this mode doesnt support headerfield but supports the header=<json> field
                ...
                if method.lower() in ("get","g"):
                    Result = get(uri,sessionKey,cert,verify,token,user_headers,data,user,passwd,timeout)
                if method.lower() in ("head","h"):
                    Result = head(uri,sessionKey,cert,verify,token,user_headers,data,user,passwd,timeout)
                if method.lower() in ("patch"):
                    Result = patch(uri,sessionKey,cert,verify,token,user_headers,data,user,passwd,timeout)
                if method.lower() in ("post","p"):
                    Result = post(uri,sessionKey,cert,verify,token,user_headers,data,user,passwd,timeout)
                if method.lower() in ("put"):
                    Result = put(uri,sessionKey,cert,verify,token,user_headers,data,user,passwd,timeout)
                if method.lower() in ("delete","del","d"):
